...
function create(brass, options) {
    ...
    rpm.brass = brass;
    rpm.options = assign({}, defaults, options || {});
...
function build(instance) {
    return butil.stream(function (file, callback) {
        ...
        args = [];
        args.push('rpmbuild', '-bb', '--quiet');
        if (instance.options.target) {
            args.push('--target', instance.options.target);
        }
        args.push('--buildroot', instance.buildRoot, file.relative);

        command = args.shift();

        proc = spawn(command, args, { cwd: instance.buildDir_SPECS });
        proc.on('exit', function (code, signal) {
            var error;
            if (code !== 0) {
                error = command +' process';
                if (signal) {
                    error += ' received signal '+ signal +' and';
                }
                error += ' exited with return code '+ code;
                error = butil.error(error);
            }
            callback(error);
        });
        ...
        proc.stderr.on('data', function (error) {
            gutil.log(butil.error(error).toString());
        });
    });
}
...
function _assign() {
    ...
    return args.reduce(function (coll, next) {
        return util._extend(coll, next);
    });
}
